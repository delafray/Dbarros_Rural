-- Helper RPC para o sistema de backup automatico.
-- Retorna schema completo: tabelas + colunas + deps FK + funcoes + politicas RLS.
-- Permite que o backup descubra tudo sem necessidade de lista manual.
-- Execute este arquivo no SQL Editor do Supabase uma unica vez.
-- Para atualizar: basta re-executar (CREATE OR REPLACE).
...
AS $$
  SELECT json_build_object(
    ...
    -- pg_get_functiondef() retorna o DDL exato que existe no banco
    'functions', (
      SELECT COALESCE(
        json_agg(
          json_build_object(
    ...
    -- Inclui: nome, tabela, cmd (SELECT/INSERT/ALL), roles, USING, WITH CHECK
    'policies', (
      SELECT COALESCE(
        json_agg(
          json_build_object(
