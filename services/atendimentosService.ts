...
export interface Atendimento {
    ...
    updated_at: string;
    // Joined via query
    clientes?: { razao_social: string | null; nome_completo: string | null; nome_fantasia: string | null } | null;
...
export const atendimentosService = {
    ...
    async addHistorico(entry: HistoricoInsert): Promise<void> {
        ...
        if (entry.data_retorno !== null && entry.data_retorno !== undefined) {
    ...
    /** Retorna o nome do contato para exibição */
    getContatoExibicao(a: Atendimento): string {
        return a.contatos?.nome || a.contato_nome || '—';
    },

    /** Retorna o telefone para exibição */
    getTelefoneExibicao(a: Atendimento): string {
        return a.contatos?.telefone || a.telefone || '—';
    },
};
