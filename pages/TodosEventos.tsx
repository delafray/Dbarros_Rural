                ...
                {isLoading ? (
                    ...
                    <div className="overflow-y-auto border border-slate-200 rounded-lg bg-white" style={{ maxHeight: 'calc(100vh - 150px)' }}>
                        {filteredGroups.map(([eventName, edicoesList]) => (
                            <div key={eventName}>
                                {/* Event group header */}
                                <div className="sticky top-0 z-10 flex items-center gap-2 px-3 py-1 bg-slate-200 border-b border-slate-300">
                                    ...
                                    <span className="text-[11px] font-black text-slate-900 uppercase tracking-widest flex-1">{eventName}</span>
                                    <span className="text-[9px] font-bold text-slate-500 bg-white border border-slate-300 px-1.5 py-0.5 rounded-full">{edicoesList.length}</span>
                                </div>
                                ...
                                {edicoesList.map(edicao => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {/* Edition title */}
                                            <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wide group-hover:text-blue-600 transition-colors flex-1 truncate">
                                                {edicao.titulo}
                                            </span>
